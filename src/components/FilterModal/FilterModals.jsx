...
const FilterModals = ({ isOpenFilterModal, setIsOpenFilterModal }) => {
   ...
   const handleGuestHouseProperty = (value) =>{
    ...
    useEffect(()=>{
        fetch(`https://airbnb-server-dn4czmnee-habibullahraju.vercel.app/filter?activePlaceType=${activePlaceType}&minValue=${rangMinValue}&maxValue=${rangeMaxValue}&beds=${beds}&bathrooms=${bathrooms}&homeProperty=${homeProperty}&apartmentProperty=${apartmentProperty}&guestHouseProperty=${guestHouseProperty}`)
        .then(res => res.json())
